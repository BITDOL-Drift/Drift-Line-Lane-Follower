...
class Follower:
  ...
  def image_callback(self, msg):
	...
	#   ang_z = err*0.0028
	  ang_z = (float(err) / 100)*(0.15) + ((err - perr)/(rospy.get_time() - ptime))*1/20/100 #+ (serr*dt)*1/20/100 #1 is best, starting 3 unstable
	...
	#   print(cx, err*0.02, ang_z)
	  print("cx:{}, err:{:.4f}, ang_z:{:4f}, lin_x:{:4f}".format(cx, err*0.0015, ang_z, lin_x))
	  serr = err + serr
	  perr = err
